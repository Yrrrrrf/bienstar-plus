...
<div class="prose prose-xl container mx-auto p-4">
    ...
    <div class="flex justify-between items-center mb-6 not-prose">
        <h1 class="text-2xl font-bold m-0">{m.medic_view()} - {m.patient_history()}</h1>
        <div class="flex gap-2">
            <button class="btn btn-secondary btn-sm" onclick={() => vitalsStore.simulateReading()}>
                {m.simulate_beat()}
            </button>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 not-prose">
        <!-- Timeline Column -->
        <div class="md:col-span-1 card bg-base-100 shadow-xl border-2 border-base-200">
            <div class="card-body p-4">
                <h2 class="text-xl font-semibold mb-4 card-title">{m.alert_timeline()}</h2>
                {#if alerts.length === 0}
        ...
        <!-- Data Table Column -->
        <div class="md:col-span-2 card bg-base-100 shadow-xl border-2 border-base-200">
             <div class="card-body p-4">
                <h2 class="text-xl font-semibold mb-4 card-title">{m.recent_readings()}</h2>
